...
layui.use(['form','jquery',"layer"],function() {
    ...
    }else{    //判断是否处于锁屏状态【如果关闭以后则未关闭浏览器之前不再显示】,只有登录了才提示(indexof(index)>0)
        if(window.sessionStorage.getItem("lockcms") != "true" && window.sessionStorage.getItem("showNotice") != "true"&&window.location.href.indexOf("login")<0){
            showNotice();
        }
    }
    ...
    if(window.sessionStorage.getItem('userFace') &&  $(".userAvatar").length > 0){
        ...
        $("#userFace").attr("src","../../images/eva.png");
    }
    ...
    function showNotice(){
        layer.open({
            ...
            btn: ['我知道了'],
            moveType: 1,
            content: '<div style="padding:15px 20px; text-align:justify; line-height: 22px; text-indent:2em;border-bottom:1px solid #e2e2e2;"><p class="layui-red">请使用前请务必仔细阅读首页右下角的《更新日志》，避免使用中遇到一些简单的问题造成困扰。</p></pclass></p><p>温馨提示：<br/><span class="layui-red">请勿进行出售或者上传到任何素材网站，否则将追究相应的责任。</span></p></div>',
    ...
    function lockPage(){
        layer.open({
            ...
            content : '<div class="admin-header-lock" id="lock-box">'+
                            '<div class="admin-header-lock-img"><img src="images/eva.png" class="userAvatar"/></div>'+
                            '<div class="admin-header-lock-name" id="lockUserName">Mikoto</div>'+
                            '<div class="input_btn">'+
    ...
    function setName(){
        ...
        $("#lockUserName").text(window.sessionStorage.getItem("username"));
    ...
    $(".signOut").click(function(){
        ...
        $.post("/user/logout",{
           ...
           /* if (typeof  res=="string"){//如果返回的是字符串则转为对象
                res = JSON.parse(res);
            }*/
        ...
        layer.open({
            ...
            success : function(index, layero){
                ...
                form.on('switch(multipleLogin)', function(data){
                    layer.tips('温馨提示：此功能暂未开放，敬请谅解', data.othis,{tips: 1})
                });
                form.render();  //表单渲染
            }
        })
